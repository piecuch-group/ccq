subroutine t3d_update(n0, n1, n2, n3, k1, k2, k3, k4, lvl_q, shift, v3d, &
   ...
   call sum_shift(4, shape(intm), size(intm), shape(x10), &
                  size(x10), (/n0 - n0, n0 - n0, n1 - n0, n0 - n0/), '2134', 1.000, intm, x10)
   ...
   if (lvl_q) then
      ...
      do i = n0 + 1, n2 - 2
         do j = i + 1, n2 - 1
         do k = j + 1, n2                     !ilias: if no quadruples comment out the following 14 lines
            if (indocc(k, j, i) .eq. 1) cycle
            !$omp parallel default(none), &
   ...
   call sum_shift(4, shape(intm), size(intm), shape(x11), &
                  size(x11), (/n0 - n0, n2 - n0, n1 - n0, n2 - n0/), '1432', 1.000, intm, x11)
   ...
   call sum_shift(4, shape(intb), size(intb), shape(x13), &
                  size(x13), (/n0 - n0, n0 - n0, n2 - n0, n0 - n0/), '2143', 1.000, intb, x13)
   ...
   call sum_shift(4, shape(intb), size(intb), shape(x14), &
                  size(x14), (/n0 - n0, n2 - n0, n2 - n0, n2 - n0/), '4132', 1.000, intb, x14)
   ...
   if (t2diag3 .ne. 0) then
      ...
      do i = n0 + 1, n2 - 2
         do j = i + 1, n2 - 1
         do k = j + 1, n2                      !ilias: commented out 29 lines for 3cc
            if (indocc(k, j, i) .eq. 1) cycle
            !$omp parallel default(none), &
   ...
   if (t3diag3 .ne. 0) then
      ...
      do i = n0 + 1, n2 - 2
         do j = i + 1, n2 - 1
         do k = j + 1, n2                      !ilias: commented out 20 lines for 3cc
            if (indocc(k, j, i) .eq. 1) cycle
            !$omp parallel default(none), &
