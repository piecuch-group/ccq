...
C
       ...
       call reorder_shift(4,shape(IntM),size(IntM),shape(D1),size(D1),
     & (/N1-n0,N2-n0,N2-n0,N0-n0/),'2314',IntM,D1)
       allocate(B2(N1+1:N3,N0+1:N1))
       call reorder_stripe(2,shape(t1A),size(t1A),'12',t1A,B2)
       ...
       deallocate(B2)
C
       call sum_stripe(4,shape(V2C2A),size(V2C2A),'4123',
     & 1.000,V2C2A,U1)
       deallocate(U1)
C
       allocate(D1(N0+1:N2,N1+1:N3,N2+1:N3,N0+1:N1))
       call reorder_shift(4,shape(IntM),size(IntM),shape(D1),size(D1),
     & (/N0-n0,N1-n0,N2-n0,N0-n0/),'3214',IntM,D1)
       ...
       deallocate(D2)
C
       call sum_stripe(4,shape(V2C2A),size(V2C2A),'1423',
     & -1.000,V2C2A,U3)
       deallocate(U3)
C
       end
